...
def main():
    ...
    endpoint_genes = endpoint_genes[endpoint_genes['p_val_adj'] < 0.05]
    endpoint_genes['Gene'] 

    # Filtering tf_tg based on conditions
    contextulised_tf_tg = tf_tg[tf_tg['source_genesymbol'].isin(genes) & tf_tg['target_genesymbol'].isin(endpoint_genes['Gene'])]
    ...
    genes = genes.rename(columns={'Gene':'target_genesymbol'})

    # Join the tf-deg network with the deg lfc values
    tfs = contextulised_tf_tg
    tfs2 = tfs.merge(genes, on='target_genesymbol')
    tfs2_filtered = tfs2[['source', 'target', 'consensus_stimulation', 'avg_log2FC']].copy()
    tfs2_filtered['exp_sign'] = np.where(tfs2_filtered['consensus_stimulation'] == True, tfs2_filtered['avg_log2FC'], -1*(tfs2_filtered['avg_log2FC']))
